...
impl BytePoly {
    ...
    pub fn from_hex(hex: &str) -> Self {
        ...
        let out = Self::from_coeffs_iter(bytes.into_iter().map(i32::from));
        eprintln!("from_hex({hex}) -> {out:?} ");
        out
    }
    ...
    pub fn compute_digest<F: Field>(
        ...
        for wit in witness.iter() {
            for chunk in wit.borrow().0.chunks(CHECKED_COEFFS_PER_POLY) {
                for (k, coeff) in cur.iter_mut().enumerate() {
                    *coeff = *coeff * F::Elem::from_u64(1u64 << BITS_PER_COEFF)
