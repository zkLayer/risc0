...
impl CallNode<'a> {
    fn walk(&self) -> impl Iterator<(&'a [u32], usize)> + 'a {}
}
...
impl Profiler {
    ...
    pub fn new(filename: &str, elf_data: &[u8]) -> Result<Self> {
        ...
        let bin_name = profiler.profile.get_string_id(filename);
                ...
                TraceEvent::Instruction { cycle, pc, insn } => {
                    ...
                    *top.counts.entry(orig_pc).or_insert(0) += cycles as usize;
                    match extract_call_stack_op(orig_insn) {
                        Some(CallStackOp::Push) => {
                            self.curr_stack.push(top.calls.entry(orig_pc).or_default());
                        }
    ...
    pub fn finalize(self) -> proto::Profile {
        ...
        for (pc, count) in self.counts.iter() {
            ...
            let loc = proto::Location {
                ...
                line: frames
                    .into_iter()
                    .map(|fr| proto::Line {
                        function_id: self.profile.get_function_id(&fr.name, &fr.filename),
                        line: fr.lineno,
                    })
                    .collect(),
                ..Default::default()
            ...
            let sample = proto::Sample {
                location_id: vec![self.profile.get_location_id(loc)],
...
impl ProfileBuilder {
    fn new() -> Self {
        ...
        assert_eq!(0, builder.get_string_id(""));

        // Set up defaults for us
        let sample_type = proto::ValueType {
            r#type: builder.get_string_id("cycles"),
            unit: builder.get_string_id("count"),
            ..Default::default()
        };
    ...
    /// Gets the id for the given location, inserting it in the table if needed.
    fn get_location_id(&mut self, mut loc: proto::Location) -> u64 {
    ...
    /// Gets the id for the given function, inserting it in the table if needed.
    fn get_function_id(&mut self, name: &str, filename: &str) -> u64 {
        ...
        let name = self.get_string_id(name);
        let filename = self.get_string_id(filename);
    ...
    /// Gets the id for the given string, inserting it in the table if needed.
    fn get_string_id(&mut self, s: &str) -> i64 {
